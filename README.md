...
## EDA графа устройств (Device Fingerprint)

Мы провели глубокий анализ возможности использования графовых фичей на основе device fingerprint.

### H1: Разделение устройств между клиентами

![H1 Device Sharing](images/h1_device_sharing.jpg)

- 58.4% устройств используется 2+ клиентами (shared devices)
- 99.9% клиентов хотя бы раз использовали shared-устройство
- Топ устройство: 4,531 уникальных клиентов, 1.3M операций

### H2 + H6: Каузальный лифт и проверка каузальности

![H2+H6 Causal Lift](images/h2_h6_causal_lift.jpg)

- Каузальный кросс-клиентский лифт: 0.00x (RED другого клиента на том же устройстве ДО текущего события не повышает риск)
- 94.4% ассоциаций каузальны (прошли проверку)
- **Вывод**: прямое заражение устройств не работает как сигнал

### H3: Кластеризация фрода в графе устройств

![H3 Fraud Clustering](images/h3_fraud_clustering.jpg)

- 73.6% shared-устройств содержат хотя бы одного RED-клиента (vs ожидаемых 36.9%)
- Коэффициент кластеризации: 2.00x от случайного
- Фрод действительно кластеризуется на shared-устройствах

### H4: Сигнал мерчантов (event_desc)

![H4 Merchant Signal](images/h4_merchant_signal.jpg)

- 0 надежных высокорискованных мерчантов (>3x baseline)
- Все мерчанты имеют RED rate <=175% baseline
- Сигнал мерчантов не работает как самостоятельная фича

### H5: Покрытие device_fp_i по датасетам

![H5 Coverage](images/h5_coverage.jpg)

- pretrain+train: 16.2% транзакций с валидным device_fp_i
- test: 54.2% покрытие (растет со временем)
- RED-транзакций с device_fp_i: 26.5%

### Финальный вердикт: 3/6 гипотез прошли

| Гипотеза | Порог | Результат |
|---|---|---|
| H1 sharing | >15% устройств 2+ клиентов | ПРОШЁЛ |
| H2 lift | >2.0x каузальный лифт | ПРОВАЛ |
| H3 cluster | >1.5x случайного | ПРОШЁЛ |
| H4 merchant | >5 надежных HR мерчантов | ПРОВАЛ |
| H5 coverage | >40% RED-событий | ПРОВАЛ |
| H6 causal | >80% каузальных | ПРОШЁЛ |

**-> Умеренный сигнал: GNN возможен, но прирост ожидается небольшой. Мы использовали упрощенные граф-признаки (device fingerprint система) вместо полноценного GNN.**
